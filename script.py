...
# Function for Pygame
def pygame_plot(explored, optimal_path):
    pygame.init()
...
# Function for finding the obstacle area
def obstacle_area():
    obstacle = []
    ...
    if queue != goal:
        ...
        if current_position not in visited_nodes and current_position not in obstacle_space:
            for i in range(queue_nodes.qsize()):
                if queue_nodes.queue[i][3] == current_position:
                    if queue_nodes.queue[i][0] > cost2_come:
                        queue_nodes.queue[i][0] = (cost2_come, index, parent_node, current_position)
                        path_dict[queue_nodes.queue[i][3]] = pos
                        return
                    else:
                        return
...
while True:
    ...
    if position != node_state_g:
        ...
        if (position[0]+1) <= 600 and (position[1]+1) <= 250:
            ActionMoveUpRight(position, queue_pop, node_state_g)
        if (position[0]-1) >= 0 and (position[1]+1) <= 250:
            ActionMoveUpLeft(position, queue_pop, node_state_g)
        if (position[0]+1) <= 600 and (position[1]-1) >= 0:
...
print('Total timetaken:', Total_time, 'seconds')
pygame_plot(visited_nodes, best_path)
